...
- **A relay alone still leaks the IP.** `/ohttp-keys` is fetched from the **gateway**, not through the relay (`OhttpClient` only routes the encapsulated request), so a cold start contacts the gateway directly even with a relay configured. Pinning `publicKeyConfig` — `OHTTP_PROVER_KEY_CONFIG` / `OHTTP_DISCOVERY_KEY_CONFIG` — stops the key fetch happening at all. Set a relay without pinning the config and you have moved the leak, not closed it. The server warns when it sees that combination.
- **One relay fronts one gateway.** The encapsulated request names no destination — the client does `fetch(relayUrl ?? gatewayUrl, init)` — so the relay forwards to whichever gateway it was configured with. Fronting both the prover and the discovery service needs two relay endpoints, which may be two paths on one deployment. `OHTTP_RELAY_URL` is deprecated for this reason.
- **This blinds the operator, not the user.** On the SDK path the OHTTP client is *our server*, so a relay hides our egress IP from StarkWare. Users are already behind the MCP hop, which §3 names as the open gap. On the wallet route the wallet proves, so its OHTTP configuration is the wallet's business, not ours.
