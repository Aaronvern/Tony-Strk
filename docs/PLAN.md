...
3. **Wallet / payment service** (the money) — assembles STRK20 actions, has them proved, and submits them. **Keys and proving stay client-side**, never on our server.

   ⚠️ **The submission path is the AVNU paymaster, and it is load-bearing** (verified — `SPIKE-RESULTS.md` §20). Proving and submitting are separate steps: the SDK's `execute()` returns `{callAndProof, registry}` — it proves but does **not** submit. Submitting means calling `apply_actions` on the pool *with the proof attached to the transaction*, which a plain starknet.js `Account` cannot do. Only two things can: the **AVNU paymaster** (`apply_action`, or **`invoke_and_apply_action` whenever a deposit is involved**, since the ERC-20 `approve` must execute as the *user* rather than the paymaster) or a STRK20-aware wallet's `executeWithProof`. So the paymaster is not an optional gasless nicety — for a server-side agent it is *how you transact at all*. It still delivers the fee privacy we wanted; that is now a bonus rather than the reason. We never custody the spending key or viewing key. Covers shield (`DEPOSIT`), private send (`TRANSFER`), unshield (`WITHDRAW`), `strk20Balances`, and the AVNU `sponsored_private` paymaster. The spec even defines a `PRIVACY_LEAK` error — the wallet refuses privacy-leaking operations.
