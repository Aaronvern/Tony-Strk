...
| 2 | **Browser** — canvas/device fingerprint, cookies, logins | **Ephemeral hardened headless browser** (Playwright), fresh profile per task, zero persistent storage, **no logins** | your device identity | fingerprinting is an arms race — "very good," never "perfect" |
...
| **Playwright + anti-fingerprint defaults** | disposable, hardened headless browser (channel 2) |
